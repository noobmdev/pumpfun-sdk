...
export function generate_transactions(serializedTransactions: Array<string>) {
	const transactionBuffers = serializedTransactions.map((transaction) =>
		Buffer.from(transaction, "base64")
	);
	const rawTransactions = transactionBuffers.map((transactionBuffer) =>
...
export async function getOptimalPriceAndBudget(
	hydratedTransaction: Transaction,
	connection: Connection
) {
...
export async function ConstructOptimalTransaction(
	prevTx: Transaction,
	connection: Connection,
	fee: number
): Promise<Transaction> {
...
export async function parseSignatures(
	...
	const parsedSignatures = await connection.getParsedTransactions(signatures, {
...
export const onBundleResult = (c: SearcherClient): Promise<[number, any]> => {
	console.log("🚀 ~ file: index.ts:347 ~ onBundleResult ~ c:", c);
	return new Promise((resolve) => {
		let state = 0;
		...
		const listener = c.onBundleResult(
			...
			(result) => {
				console.log(
					"🚀 ~ file: index.ts:354 ~ returnnewPromise ~ result:",
					result
				);
...
export async function getMintPoolData(
	connection: Connection,
	program: anchor.Program<Idl>,
	user: PublicKey,
	txId: string
): Promise<PoolData | undefined> {
	let neededInstruction:
		| PartiallyDecodedInstruction
		| ParsedInstruction
		| null = null;
	...
	for (var i = 0; i < parsed_sigs.length; i++) {
		try {
			...
			for (let ix of instructions) {
				try {
					const hasNeededProgramId = ix.programId.toBase58() == PROGRAM_ID;
					//@ts-ignore
					if (!ix.accounts) {
						continue;
					}
			...
			const realTokenReserves = (
				bondingCurveData.realTokenReserves as any
			).toNumber();
			const realSolReserves = (
				bondingCurveData.realSolReserves as any
			).toNumber();
			const adjustedVirtualTokenReserves =
				virtualTokenReserves / 10 ** decimals;
			const adjustedVirtualSolReserves = virtualSolReserves / LAMPORTS_PER_SOL;
			const virtualTokenPrice =
				adjustedVirtualSolReserves / adjustedVirtualTokenReserves;
